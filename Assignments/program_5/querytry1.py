...
class mongoHelper(object):
    def __init__(self):
        ...
        self.db_airport = self.client.world_data.airport
    ...
    def get_doc_by_keyword(self,db_name,field,key):
        ...
        if db_name == 'airports':
            res = self.db_airport.find_one({field : key})

        return res

    def get_nearest_neighbor(self,lon,lat,r):
       # air_res = self.db_ap.find( { 'geometry' : { '$geoWithin' : { '$geometry' : poly } } })
        air_res = self.db_airport.find( { 'geometry': { '$geoWithin': { '$centerSphere': [ [lon, lat ] , r / 3963.2 ] } }} )
